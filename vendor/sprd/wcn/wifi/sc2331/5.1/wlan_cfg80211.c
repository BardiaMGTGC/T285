...
static unsigned char *get_ie_entry(unsigned char *ie, unsigned int ie_len, unsigned char eid)
{
...
const u8 * wl_bss_get_ie(u8 *ies, u16 ies_len, u8 ie)
{
	const u8 *end, *pos;
...
int wl_update_bss_info(wlan_vif_t *vif, u8 *ies, u16 ies_len)
{
	struct cfg80211_bss *bss = NULL;
	...
	struct wiphy *wiphy;
	buf_scan_frame_t *scan_frame;
	int i, freq, signal, mgmt_len;
	bool found = false;
	struct timespec ts;
	...
	if(!bss) {
		printkd("inform and put bss\n");
		mgmt = kmalloc(1024, GFP_KERNEL);
		if(!mgmt) {
			printke("failed to kmalloc mgmt\n");
			return -ENOMEM;
		}

		printke("build a probe resp\n");

		mgmt->frame_control = cpu_to_le16(IEEE80211_FTYPE_MGMT |
                                                 IEEE80211_STYPE_PROBE_RESP);
		mgmt->duration = cpu_to_le16(0);
		memcpy(mgmt->da, vif->ndev->dev_addr, ETH_ALEN);
		memcpy(mgmt->sa, vif->cfg80211.bssid, ETH_ALEN);
		memcpy(mgmt->bssid, vif->cfg80211.bssid, ETH_ALEN);
		...
		mgmt->u.probe_resp.capab_info = cpu_to_le16(WLAN_CAPABILITY_ESS);

		memcpy(mgmt->u.probe_resp.variable, ies, ies_len);
		mgmt_len = 24 + 12 + ies_len;

		/*if rsn exist, update rsn ie*/
		band = wiphy->bands[IEEE80211_BAND_2GHZ];
		freq = ieee80211_channel_to_frequency(9, band->band);
...
{
	unsigned char *req_ie_ptr, *resp_ie_ptr, *bssid_ptr, *pos, *value_ptr, *rsn_ies, *tmp_ies;
	...
	/* rsn ie */
	rsn_ies = resp_ie_ptr + resp_ie_len * 2 - 1;
	...
	if ( (vif->cfg80211.connect_status == ITM_CONNECTING) || (1 == reassociate_rsp_flag) )
	{
		...
		if(memcmp(bssid_ptr, vif->cfg80211.bssid, 6)) {
			printke("connect to a different BSSID\n");
			memcpy(vif->cfg80211.bssid, bssid_ptr, 6);
		}
		tmp_ies = kmalloc(req_ie_len, GFP_KERNEL);
		if(!tmp_ies) {
			printkd("%s(), failed to kmalloc tmp_ies\n", __func__);
			kfree(pos);
			goto out;
		}
		memcpy(tmp_ies, req_ie_ptr, req_ie_len);
		rsn_ie = wl_bss_get_ie(tmp_ies, req_ie_len, WLAN_EID_RSN);
		if(rsn_ie){
				printke("find rsn ie, update rsn capab\n");
				rsn_ie_len = rsn_ie[1];
				rsn_ie[rsn_ie_len] = rsn_ies[7];
				rsn_ie[rsn_ie_len+1] = rsn_ies[8];
		}
		wl_update_bss_info(vif, tmp_ies, req_ie_len);
		if(tmp_ies)
			kfree(tmp_ies);
...
#define SSID_LEN_OFFSET		(37)
	...
	if(vif->auto_channel == 1) {
		unsigned char *dsset_entry = NULL;
		unsigned char *ie = (unsigned char*)mgmt +
				(WLAN_BEACON_IE_OFFSET + WLAN_MGMT_FRAME_HEAD_LEN);
		int ie_len = beacon->head_len -
				(WLAN_BEACON_IE_OFFSET + WLAN_MGMT_FRAME_HEAD_LEN);
		dsset_entry = get_ie_entry((unsigned char*)ie, ie_len,
							WLAN_EID_DSSET_IE);

		if(dsset_entry) {
			/* Set channel as 0 */
			printkd("set channel 0 to start auto channel selection\n");
			dsset_entry[2] = 0;
			vif->auto_channel = 0;
		}
	}
